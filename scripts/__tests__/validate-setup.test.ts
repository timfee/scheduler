...
describe("validate-setup batch insert", () => {
  ...
  it("should successfully batch insert appointment types", () => {
    ...
    try {
      ...
      const sortedTypes = appointmentTypes.sort(
  ...
  it("should be more efficient than individual inserts", () => {
    ...
    try {
      ...
      db.insert(schema.appointmentTypes).values(batchData).run();
      ...
      for (const item of batchData) {
        db.insert(schema.appointmentTypes).values(item).run();
      }
      const individualEnd = performance.now();

      // Verify both methods produce the same result
      const afterBatch = 3;
      const afterIndividual = db
        .select()
