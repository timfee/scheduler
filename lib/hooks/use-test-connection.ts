...
import { type CalendarOption } from "@/actions/connections-actions";
import { testConnectionAction, listCalendarsAction } from "@/actions/connections-actions";
...
export function useTestConnection(): UseTestConnectionReturn {
  ...
  const testConnection = useCallback(async (form: UseFormReturn<ConnectionFormValues>) => {
    ...
    try {
      ...
      const fetchedCalendars = await listCalendarsAction(values.provider, testData);
