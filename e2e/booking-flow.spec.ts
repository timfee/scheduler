...
test.describe('Booking Flow E2E', () => {
  ...
  test('complete booking flow', async ({ page }) => {
    ...
    await expect(page.locator('h1')).toContainText(/book|appointment|schedule/i);
    
    // Look for appointment type selection
    const typeButtons = page.locator('button:has-text("intro"), button:has-text("Introduction"), button:has-text("30 min")');
    ...
    const availableDates = page.locator('button:not([disabled]):not(:has-text("busy"))');
    ...
    const availableSlots = page.locator('button:not([disabled]):has-text(/^\\d{1,2}:\\d{2}/)');
    ...
    await page.fill('input[name="email"], input[placeholder*="email"]', 'john@example.com');
    
    // 5. Confirm booking
    const confirmButton = page.locator('button:has-text("Confirm"), button:has-text("Book"), button:has-text("Submit")');
    await expect(confirmButton).toBeVisible();
    await confirmButton.click();
    
    // 6. Verify success
    await expect(page.locator('text=confirmed, text=success, text=booked')).toBeVisible({ timeout: 10000 });
  });
  ...
  test('handles validation errors', async ({ page }) => {
    ...
    if (await confirmButton.isVisible()) {
      ...
      await expect(page.locator('text=required, text=error, .error, [role="alert"]')).toBeVisible();
    }
  });
  ...
  test('displays loading states', async ({ page }) => {
    ...
    const loadingIndicators = page.locator('[role="status"], .loading, .spinner, text=loading');
  ...
  test('handles no available slots', async ({ page }) => {
    ...
    const noSlotsMessage = page.locator('text=no times available, text=no slots, text=fully booked');
  ...
  test('handles network errors gracefully', async ({ page }) => {
    ...
    const errorMessages = page.locator('text=error, text=failed, text=try again, [role="alert"]');
