...
test.describe("Connections E2E", () => {
  ...
  test("should load connections page without import errors", async ({
    ...
    const hasUpdateConnectionError = errors.some(
      (error) =>
        error.includes("updateConnectionAction") && error.includes("not found"),
    );

    expect(
      hasUpdateConnectionError,
  ...
  test("should be able to create a new connection", async ({ page }) => {
    ...
    const addButton = page
      .locator("text=Add Connection")
      .or(page.locator("text=New Connection"))
      .or(page.locator("button").filter({ hasText: /add|new|create/i }));
  ...
  test("should handle connection form submission", async ({ page }) => {
    ...
    const exportErrors = consoleErrors.filter(
      (error) =>
