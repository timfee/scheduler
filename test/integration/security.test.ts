...
describe("Security Tests", () => {
  ...
  describe("SQL Injection Prevention", () => {
    ...
    it.each(sqlInjectionPayloads)(
      "should safely handle SQL injection in name field: %s",
      async (payload) => {
    ...
    it("should validate and sanitize input before database operations", async () => {
      ...
      expect(() => mockValidation(bookingData)).toThrow(
  ...
  describe("XSS Prevention", () => {
    ...
    it.each(xssPayloads)(
      "should escape XSS payload in name field: %s",
      async (payload) => {
        const bookingData = bookingFactory.build({
          name: payload,
          email: "test@example.com",
    ...
    it.each(xssPayloads)(
      "should escape XSS payload in email field: %s",
      async (payload) => {
        const bookingData = bookingFactory.build({
          name: "John Doe",
          email: payload,
  ...
  describe("Rate Limiting", () => {
    it("should enforce rate limit per email", async () => {
      ...
      const mockActionWithRateLimit = jest
        .fn()
        .mockImplementation(async (data: BookingFormData) => {
          const now = Date.now();
      ...
      await expect(mockActionWithRateLimit(bookingData)).rejects.toThrow(
        "Too many booking attempts",
      );

      // Different email should succeed
      const differentEmailData = bookingFactory.build({
        email: "other@example.com",
      });
      await expect(
        mockActionWithRateLimit(differentEmailData),
    ...
    it("should reset rate limit after time window", async () => {
      ...
      try {
        // First request
        await expect(
          mockActionWithRateLimit(bookingData),
        ).resolves.not.toThrow();
    ...
    it("should handle concurrent rate limit checks", async () => {
      ...
      const requests = Array(5)
        .fill(null)
        .map(() => mockActionWithRateLimit(bookingData));
  ...
  describe("Authorization", () => {
    it("should prevent unauthorized access to booking data", async () => {
      ...
      const checkAuthorization = jest
        .fn()
        .mockImplementation((userRole: string) => {
