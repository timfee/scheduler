...
function TestComponent() {
  ...
  return (
    <div>
      ...
      <button onClick={() => updateBookingStep({ type: "intro" })}>
        set type
      </button>
      <button
        onClick={() =>
          updateBookingStep({ selectedDate: new Date("2024-01-01") })
        }
      >
        set date
      </button>
      <button onClick={() => updateBookingStep({ selectedTime: "10:00" })}>
        set time
      </button>
      <button
        onClick={() =>
...
describe("booking flow parallel routes", () => {
  it("URL state updates correctly for type", () => {
    ...
    expect(div.querySelector('[data-testid="type"]')?.textContent).toBe(
      "intro",
    );
  });
  ...
  it("tracks progress correctly", () => {
    ...
    expect(div.querySelector('[data-testid="progress"]')?.textContent).toBe(
      "0",
    );
    expect(div.querySelector('[data-testid="complete"]')?.textContent).toBe(
      "false",
    );
  ...
  it("completes booking when all fields are set", () => {
    ...
    expect(div.querySelector('[data-testid="time"]')?.textContent).toBe(
      "10:00",
    );
  });
