...
let createBookingAction: jest.MockedFunction<(data: BookingFormData) => Promise<void>>;
let clearRateLimiter: jest.MockedFunction<() => void>;

// Track booking attempts and their outcomes
const bookingAttempts = new Map<string, { success: boolean; timestamp: number }>();
...
describe('Booking Race Conditions', () => {
  ...
  describe('Concurrent booking attempts', () => {
    it('should prevent double booking with Promise.all simulation', async () => {
      ...
      const booking2 = createBookingAction({ ...bookingData, email: 'user2@example.com' });
    ...
    it('should handle rapid sequential bookings', async () => {
      ...
      const bookingPromises = Array(10).fill(null).map((_, i) => 
        createBookingAction({
          ...baseBookingData,
  ...
  describe('Lock timeout and cleanup', () => {
    it('should release locks after timeout', async () => {
      ...
      const slowMockCreateBooking = async (data: BookingFormData): Promise<void> => {
        // Simulate a slow calendar API call
        await new Promise(resolve => setTimeout(resolve, 2000));
        
        const slotKey = `${data.date}-${data.time}`;
        const attemptId = `${data.email}-${Date.now()}`;
        bookingAttempts.set(attemptId, { success: true, timestamp: Date.now() });
      };

      const fastMockCreateBooking = async (data: BookingFormData): Promise<void> => {
      ...
      const slowBooking = slowMockCreateBooking({ ...bookingData, email: 'slow@example.com' });
      
      // Wait a bit
      await new Promise(resolve => setTimeout(resolve, 100));
      
      // Second booking waits, then proceeds after timeout
      const fastBooking = fastMockCreateBooking({ ...bookingData, email: 'fast@example.com' });
    ...
    it('should clean up locks on error', async () => {
      const errorMockCreateBooking = async (data: BookingFormData): Promise<void> => {
        throw new Error('Calendar API error');
      };

      const successMockCreateBooking = async (data: BookingFormData): Promise<void> => {
        const slotKey = `${data.date}-${data.time}`;
        const attemptId = `${data.email}-${Date.now()}`;
        bookingAttempts.set(attemptId, { success: true, timestamp: Date.now() });
      };
      ...
      const errorBooking = errorMockCreateBooking({ ...bookingData, email: 'error@example.com' });
      ...
      const successBooking = successMockCreateBooking({ ...bookingData, email: 'success@example.com' });
  ...
  describe('Booking state consistency', () => {
    it('should maintain consistent state across concurrent operations', async () => {
      ...
      const bookingPromises = Array(5).fill(null).map((_, i) => 
        createBookingAction(bookingFactory.build({
          date: '2024-01-15',
          time: `${10 + i}:00`,
          type: 'intro',
          email: `user${i}@example.com`
    ...
    it('should handle mixed success and failure scenarios', async () => {
      const sameSlotBookings = Array(3).fill(null).map((_, i) => 
        createBookingAction(bookingFactory.build({
          date: '2024-01-15',
      ...
      const differentSlotBookings = Array(2).fill(null).map((_, i) => 
        createBookingAction(bookingFactory.build({
          date: '2024-01-15',
  ...
  describe('Performance under load', () => {
    it('should handle high concurrency without performance degradation', async () => {
      ...
      const bookingPromises = Array(50).fill(null).map((_, i) => 
        createBookingAction(bookingFactory.build({
          date: '2024-01-15',
